...
export default function Navbar({ view, navigateTo, activeSection, setActiveSection }) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 md:px-12 transition-all duration-300 border-b ${
      scrolled 
      ...
      <a 
        href="#home" 
        className="font-extrabold text-xl md:text-2xl text-text-main dark:text-dark-text-main flex items-center gap-1.5 tracking-tight transition-transform active:scale-95" 
      ...
      >
        <span className={`w-6 h-0.5 bg-text-main dark:bg-dark-text-main transition-all duration-300 origin-center ${
          mobileMenuOpen ? 'rotate-45 translate-y-2' : ''
        }`}></span>
        <span className={`w-6 h-0.5 bg-text-main dark:bg-dark-text-main transition-all duration-300 ${
        ...
        {navItems.map((item) => {
          ...
          return (
            <li key={item.id} className="relative group">
              <a
                href={`#${item.id}`}
                className={`font-semibold text-[0.98rem] transition-colors duration-300 block py-2 md:py-0 ${
                  isItemActive 
                    ? 'text-accent' 
              ...
              <span className={`absolute bottom-[-4px] left-0 h-[2px] bg-accent transition-all duration-300 ${
